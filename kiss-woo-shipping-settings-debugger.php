...
// --- Internal classes for the scanner. MUST be defined before traits are included. ---
namespace KISSShippingDebugger {
    ...
    class HookedFunction {
        ...
        private int $line;
        public array $found_nodes = [];

        public function __construct(string $hook_name, string $function_name, int $line) {
            $this->hook_name = $hook_name;
            $this->function_name = $function_name;
            $this->line = $line;
        }
        public function getHookName(): string { return $this->hook_name; }
        public function getFunctionName(): string { return $this->function_name; }
    }
    ...
    class FunctionVisitor extends NodeVisitorAbstract {
        ...
        /** @var HookedFunction[] */
        private array $hookedFunctions = [];
        private array $shippingHooks = [
        ...
        public function enterNode(Node $node) {
            ...
            if ($node instanceof Function_) {
                $this->allFunctions[$node->name->toString()] = $node;
            }

            // Identify calls to add_filter or add_action for our target hooks
            if ($node instanceof FuncCall && $node->name instanceof Name) {
                $callName = $node->name->toString();
                if ($callName === 'add_filter' || $callName === 'add_action') {
                    if (isset($node->args[0]) && $node->args[0]->value instanceof String_) {
                        $hookName = $node->args[0]->value->value;
                        if (in_array($hookName, $this->shippingHooks, true)) {
                            if (isset($node->args[1]) && $node->args[1]->value instanceof String_) {
                                $functionName = $node->args[1]->value->value;
                                $this->hookedFunctions[] = new HookedFunction($hookName, $functionName, $node->getLine());
                            }
                        }
                    }
                }
...
// --- Return to the global namespace for the main plugin logic ---
namespace {
    ...
    class KISS_WSE_Debugger {
        ...
        public function render_page(): void {
            // Load PHP-Parser only when this page is rendered to prevent conflicts.
            if ( ! class_exists( \PhpParser\ParserFactory::class ) ) {
                $this->maybe_require_parser_loader();
            }

            // If the form was submitted, use the new value and save it to the database.
            if ( isset( $_GET['wse_additional_file'] ) ) {
                $submitted_path = sanitize_text_field( wp_unslash( $_GET['wse_additional_file'] ) );
                update_option( 'kiss_wse_additional_file_path', $submitted_path );
                $additional = $submitted_path;
            } else {
                // Otherwise, load the saved value from the database.
                $additional = get_option( 'kiss_wse_additional_file_path', '' );
            }
            ...
            // Only run the scanner if the form was submitted.
            if ( isset( $_GET['wse_additional_file'] ) ) {
                try {
